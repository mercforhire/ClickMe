...
class HostTabBarViewController: BaseTabBarViewController {
    enum Tabs: Int {
        case chat
        case calendar
        case topics
//        case status
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(switchToBooking), name: Notifications.SwitchToCalendar, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(updateHostCalenderBadgeCount), name: Notifications.UpdateHostCalenderBadge, object: nil)
        ...
        appSettings.setLastUsedMode(mode: "host")
        selectedIndex = Tabs.calendar.rawValue
    }
    
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated)
        
        if !appSettings.hasBeenToHostView() {
            let vc = StoryboardManager.loadViewController(storyboard: "Host", viewControllerId: "HostFirstTimeViewController")!
            vc.modalPresentationStyle = .fullScreen
            navigationController?.present(vc, animated: true) { [weak self] in
                self?.appSettings.setHasBeenToHostView(been: true)
            }
        }
    ...
    @objc func updateChatBadgeCount(_ notification: Notification) {
        ...
        if let badgeCount = userInfo["badgeCount"] as? Int {
            if badgeCount > 0 {
    ...
    @objc func updateHostCalenderBadgeCount(_ notification: Notification) {
        guard let userInfo = notification.userInfo else { return }
        
        if let badgeCount = userInfo["badgeCount"] as? Int {
            if badgeCount > 0 {
                tabBar.items?[Tabs.calendar.rawValue].badgeValue = "\(badgeCount)"
            } else {
                tabBar.items?[Tabs.calendar.rawValue].badgeValue = nil
            }
        }
    }
    ...
    @objc func switchToSchedule(_ notification: Notification) {
        if let modalVC = UIViewController.topViewController {
            modalVC.dismiss(animated: true) { [weak self] in
                ...
                self.selectedIndex = Tabs.calendar.rawValue
                if let userInfo = notification.userInfo {
    ...
    @objc func switchToBooking(_ notification: Notification) {
        self.selectedIndex = Tabs.calendar.rawValue
    }
}
