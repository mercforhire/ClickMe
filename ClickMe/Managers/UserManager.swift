...
class UserManager {
    ...
    func likeUser(userId: Int, completion: @escaping (Bool) -> Void) {
        FullScreenSpinner().show()
        ...
        queue.async { [weak self] in
            ...
            self.api.likeUser(otherUserId: userId) { result in
                switch result {
                case .success:
                    break
                case .failure(let error):
    ...
    func unlikeUser(userId: Int, completion: @escaping (Bool) -> Void) {
        FullScreenSpinner().show()
        ...
        queue.async { [weak self] in
            ...
            self.api.unlikeUser(otherUserId: userId) { result in
                switch result {
                case .success:
                    isSuccess = true
                case .failure(let error):
